"""Testes do runner de migrações — skip-if-applied e idempotência."""

import sqlite3

from memoria.db import MIGRATIONS, run_migrations
...
class TestRunMigrations:
    ...
    def test_each_migration_has_id_and_sql(self):
        for m in MIGRATIONS:
            assert "id" in m, f"Migration missing 'id': {m}"
            assert "sql" in m, f"Migration missing 'sql': {m}"
            assert m["id"], "Migration id must not be empty"
            assert m["sql"].strip(), "Migration sql must not be empty"
