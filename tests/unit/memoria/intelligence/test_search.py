...
from memoria.search import cosine_similarity, hybrid_score, recency_score
...
class TestRecencyScore:
    def test_recent_date_returns_high_score(self):
        ...
        recent = (datetime.now(UTC).replace(tzinfo=None) - timedelta(days=1)).isoformat()
    ...
    def test_score_is_between_zero_and_one(self):
        ...
        for days in [0, 7, 30, 365, 3650]:
            date = (datetime.now(UTC).replace(tzinfo=None) - timedelta(days=days)).isoformat()
    ...
    def test_z_suffix_stripped(self):
        ...
        recent = (datetime.now(UTC).replace(tzinfo=None) - timedelta(days=1)).isoformat() + "Z"
        score = recency_score(recent)
        assert score > 0.9
